...
## Deferred from: Playwright harness setup (2026-05-08)

- **E2E auth fixture (`storageState.json`).** Initial Playwright smoke tests target unauthenticated routes only (`/`, `/login`, dashboard-redirect). Authenticated golden-path coverage (login → board renders, import drawer, apply ritual, /settings/gmail consent ceremony) needs a `storageState` fixture captured from a manual Google sign-in. Two-step approach: (1) one-off interactive script to sign in via headed Playwright and write `e2e/.auth/user.json` (gitignored); (2) per-test `test.use({ storageState })` fixture for authed specs. Google OAuth in fully-headless CI is unreliable, so the captured-state approach is the pragmatic path. ~½ day to set up; per-spec cost trivial after. [followcv/e2e/]
- **CI integration for `npm run test:e2e`.** GitHub Actions doesn't yet run Playwright. Add a job that installs Playwright browsers (`npx playwright install --with-deps chromium`), runs `npm run test:e2e`, and uploads `playwright-report/` on failure. Defer until the auth fixture lands so the first authed test can run on every PR. ~2 hours.
