...
- [x] [Review][Patch] `s3Key` leaked from `duplicateCvVersion` return value — **Obsolete:** `duplicateCvVersion` was removed entirely. The `s3Key` leak from `restoreCvVersion` remains and is tracked below.
- [ ] [Review][Patch] `s3Key` leaked from `restoreCvVersion` return value — returns the unfiltered Prisma `CvVersion` row, violating the project-context.md rule "Don't expose `s3Key` to the client and don't write Server Actions that return it." [followcv/src/actions/manage-cv.ts]
- [ ] [Review][Patch] Rename error surfaced via global Toast, not inline error per AC1 — AC1: "on error the old name is restored and **an inline error is shown**." Currently `setToast(result.error)`. [followcv/src/components/cv/CvVersionsClient.tsx]
- [ ] [Review][Patch] Disabled Delete item with tooltip is missing per AC3 (was AC4) — AC3 requires the Delete menu item to be disabled with a tooltip when a `CvSnapshot` references the version; the page Server Component must expose a `hasSnapshots` flag per row. Currently the rejection only surfaces as a post-click toast. [followcv/src/components/cv/CvVersionsClient.tsx]
- [ ] [Review][Patch] Double rename call on Enter — `commitRename → onRename → setEditingId(null) → input unmounts → onBlur fires → commitRename again`. Same hazard when clicking the actions menu trigger while editing. Add a "submitted" ref or skip the call when `trimmed === cv.name`. [followcv/src/components/cv/CvVersionsClient.tsx]
- [ ] [Review][Patch] Stale `editValue` after first rename — `useState(cv.name)` only initializes once and `CvCard` is keyed by `cv.id` so it never unmounts; subsequent edits pre-fill with the previously typed value. Sync via effect or reset on `onEditStart`. [followcv/src/components/cv/CvVersionsClient.tsx]
- [ ] [Review][Patch] No in-flight guard on Delete confirm — double-click fires two server calls; the second hits "Not found" and surfaces a misleading toast. Add a per-confirm pending flag. [followcv/src/components/cv/CvVersionsClient.tsx]
- [ ] [Review][Patch] No in-flight guard on Use-as-current — rapid clicks create N rows and bypass the cap because the cap-check runs once per call. Track `pendingActionId` and disable the item during in-flight. [followcv/src/components/cv/CvVersionsClient.tsx]
- [ ] [Review][Patch] No length cap on rename name — only `trim()` + non-empty. A megabyte-long name persists. Add a server-side max length (e.g., 200 chars). [followcv/src/actions/manage-cv.ts]
- [ ] [Review][Patch] `oldName` parameter in `handleRename` is unused on revert — the error branch deletes the override, falling back to the prop. If overrides accumulate before `router.refresh()` lands, the revert drops back to a previous successful name rather than the most recent one. Use `setNameOverrides(prev => ({ ...prev, [id]: oldName }))`. [followcv/src/components/cv/CvVersionsClient.tsx]
- [x] [Review][Patch] Story record claimed 26 new tests; diff contained 17 — **Resolved 2026-05-08:** rewrite of Task 4 in the spec now reflects the post-pivot count of 14 new tests (4 rename + 5 restore + 5 delete) and matches the committed test file.
- [ ] [Review][Patch] Focus management missing on rename Esc/save and delete confirm/cancel — focus lands on `<body>` after each transition; spec claims keyboard-navigable. Return focus to the menu trigger on cancel and to the next sensible target on save. [followcv/src/components/cv/CvVersionsClient.tsx]
