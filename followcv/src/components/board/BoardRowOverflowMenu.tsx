...
export function BoardRowOverflowMenu({ listingId, archived }: Props) {
  ...
  function handleArchiveAction(e: React.MouseEvent) {
    // The whole row is a <Link>; without these the click bubbles up the React
    // tree and navigates to /board/[listingId] (the just-archived listing).
    e.preventDefault()
    e.stopPropagation()
  ...
  async function undoAction() {
    const action = archived ? archiveListing : unarchiveListing
    const result = await action(listingId)
  ...
  return (
    <>
      ...
      {toastVisible && (
        <Toast
          message={archived ? "Listing unarchived." : "Listing archived."}
          durationSeconds={5}
          action={{
            label: "Undo",
